...
def importer_step(data_path: str) -> pandas.DataFrame:
    ...
    sample_of_100_datapoints = dataframe[['review','sentiment']].sample(n=1000, random_state=0)
    # sample_of_100_datapoints.to_csv('./sentiment-dataset/IMDB/sample_IMDB_dataset.csv', index=False)

    logging.info("Data imported successfully.")
    logging.info(f"Columns found in data: {list(sample_of_100_datapoints.columns)}")
    logging.info(f"Data: \n{sample_of_100_datapoints.head(10)}")
    
    return sample_of_100_datapoints
